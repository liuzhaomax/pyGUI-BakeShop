...
class Order_Modify(Component):
    NAME = 'ORDER_MODIFY_PAGE'
    ...
    def init_compo(self, bro, row):
        ...
        # line_cust_name_data
        self.line_cust_name_data = QLineEdit(self.layout.main_north_east)
        self.line_cust_name_data.setStyleSheet(self.ctx.style_line)
        self.line_cust_name_data.resize(self.layout.main_north_east.geometry().width() / 4, 30)
        self.line_cust_name_data.move(170, self.layout.main_north_east.geometry().height() / 2 - 30 / 2)
        ...
        if self.data.get_type() == 'Bean':
            ...
            self.line_cust_phone_data = QLineEdit(self.layout.main_north_east)
            self.line_cust_phone_data.setStyleSheet(self.ctx.style_line)
            self.line_cust_phone_data.resize(self.layout.main_north_east.geometry().width() / 4, 30)
            self.line_cust_phone_data.move(self.layout.main_north_east.geometry().width() / 4 + 360,
                                           self.layout.main_north_east.geometry().height() / 2 - 30 / 2)
        ...
        self.btn_confirm.move(self.layout.main_south.geometry().width() / 2 - 115, self.ctx.style_btn_general_space)
        self.btn_confirm.clicked.connect(lambda: self.on_click_btn_confirm())
        # btn_cancel
        btn_cancel = Button_Insignificant('Cancel')
        self.btn_cancel = btn_cancel.init_compo(self.layout.main_south)
        self.btn_cancel.resize(100, self.ctx.style_btn_general_height)
        self.btn_cancel.move(self.layout.main_south.geometry().width() / 2 + 15, self.ctx.style_btn_general_space)
        self.btn_cancel.clicked.connect(lambda: self.on_click_btn_cancel())
    ...
    def load_items(self):
        ...
        for element in self.data.get_list_of_items():
            items_id.append(element.get_item_id())
        ...
        for i in range(items_name.__len__()):
            self.load_item_frame(items_id[i], items_name[i], quantity[i], price[i])
    ...
    def on_click_btn_add_an_item(self):
        ...
        @return:
        '''
        self.load_item_frame('', '', '', '')

    def load_item_frame(self, item_id, item_name, quantity, price):
        '''
        ...
        label_item_id_data = QLabel(item_id, frame_label_item_id_data)
        ...
        line_item_name_data = QLineEdit(item_name, frame_line_item_name_data)
        ...
        line_item_quan_data = QLineEdit(quantity, frame_line_item_quan_data)
        ...
        label_item_price_data = QLabel(price, frame_line_item_price_data)
